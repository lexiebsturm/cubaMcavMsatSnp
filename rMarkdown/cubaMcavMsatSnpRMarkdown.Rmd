...
![](../figures/Figure1.png)
```{r, kableExtra, include = FALSE, message = FALSE, warning = FALSE, results = 'hide'}
if (!require("pacman")) install.packages("pacman")
pacman::p_load("kableExtra")
gpsCoords=read.csv("../rMarkdown/samplingGpsCoords.csv")

```

```{r, samplingGpsCoords, echo=FALSE}
kable(gpsCoords, col.names = (c("Sampling Site","Sampling Date", "Depth Range (m)", "ng", "Latitude", "Longitude"))) %>%
  kable_styling(bootstrap_options = c("condensed", "hover","striped"), full_width = TRUE, position = 'left')
  
```

## Figure 2: IBS dendrogram with clones
The following chunk of code uses the IBS matrix generated from [ANGSD](https://github.com/ANGSD/angsd){target="_blank"} to produce an Identity-By-State Dendrogram which we can use to ensure that technical replicates are clustering together and to identify natural clones from the SNP dataset.
...
### Microsatellites
The program [*poppr*](https://grunwaldlab.github.io/Population_Genetics_in_R/){target="_blank"} can read in microsatallite multi-locus genotypes exported from [GenAlEx](https://biology-assets.anu.edu.au/GenAlEx/Welcome.html){target="_blank"} as .csv files and can calculate a variety of distance metrics from this data. For microsatellite data with some missing loci, they recommend using Prevosti's distance.
...
[*patchwork*](https://github.com/thomasp85/patchwork){target="_blank"} is a really easy program to combine ggplot figures, collapse legends, and add letter annotations to the plots. 
...
Taking a break from making figures, here is how AMOVA and significance was calculated from the SNP dataset using *[*poppr*](https://grunwaldlab.github.io/Population_Genetics_in_R/){target="_blank"}*. The AMOVA for microsatellites was run in [GenAlEx](https://biology-assets.anu.edu.au/GenAlEx/Welcome.html){target="_blank"}.
...
ggsave("../rMarkdown/FigureS1.tiff", height = 12.5, width = 25, units = "cm", dpi = 300)
```
...
ggsave("../rMarkdown/FigureS2.tiff", width = 17, height = 16, units = "cm", dpi = 300)
ggsave("../rMarkdown/FigureS2.eps", width = 17, height = 16, units = "cm", dpi = 300)
```