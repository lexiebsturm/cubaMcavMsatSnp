...
snpPcaShallow = data.frame(row.names = rownames(snpPcaValuesShallow), sample = rownames(snpPcaValuesShallow), 
                    site = popDataShallow$population, depth = popDataShallow$depth, location = popDataShallow$location, PC1 = snpPcaValuesShallow[,1],
...
brewer.pal(8, "Dark2")
plotPops = c("Cabo Lucrecia", "Cayo Anclitas", "Cayo Jutías",
             "Cayo Sabinal", "Chivirico", "Guanahacabibes", "Isla de la Juventud")

cubasnpPcaShallowPlotA = ggplot(snpPcaShallow, aes(x = PC1, y = PC2, color = site, fill = site, shape = location)) +
  ...
  guides(fill = guide_legend(override.aes = list(shape = c(21, 21, 23, 21, 21, 23, 23), size = 4), order = 1, fill=NA), shape = F, colour="none") +
  ggtitle("SNP PCA of shallow samples") +
  theme_bw()
...
ggsave("cubasnpPcaShallowPlot.tiff", plot = cubasnpPcaShallowPlot, height = 20, width = 25, unit = "cm", dpi = 300)
